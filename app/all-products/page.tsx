...
export const metadata = {
  ...
  description: "Browse all our products from fashion, gadgets, and gaming categories"
}
...
export default async function AllProductsPage() {
  ...
  const getCategoryBadge = (category: string) => {
    const badges = {
      fashion: { color: "bg-pink-500/90", label: "Fashion" },
      gadgets: { color: "bg-blue-500/90", label: "Gadgets" },
      gaming: { color: "bg-purple-500/90", label: "Gaming" }
    }
                    ...
                    >
                      <div className="relative overflow-hidden rounded-lg bg-white dark:bg-slate-800 shadow-sm aspect-[3/4]">
                        ...
                        <div className="absolute bottom-2 left-2">
                          <span className={`inline-flex items-center gap-1 rounded-md ${badge.color} px-2 py-0.5 text-xs font-medium text-white`}>
                            {badge.label}
                          </span>
                        </div>
                      </div>
